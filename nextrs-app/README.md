...
cargo run --locked --no-default-features --features local-db --bin trust-deeds-bootstrap
cargo run --locked --no-default-features --features local-db --bin trust-deeds-nextrs
```

Open <http://localhost:3002>. An anonymous request redirects to `/login`. Sign
in with the `ADMIN_EMAIL` and `ADMIN_PASSWORD` from `.env`.

The explicit bootstrap command creates the local database, applies the
...
cargo build --release --locked --no-default-features --features local-db --bin trust-deeds-nextrs
...
Set the Vercel project Root Directory to `nextrs-app`. Configure these project
...
only the remote libSQL client. `remote-db` is the package default because the
Vercel Rust builder invokes Cargo without feature flags; local development
selects `local-db` explicitly, so the embedded SQLite core is not linked into
