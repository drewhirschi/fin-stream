...
/// Helper: set up an in-memory SQLite database with schema.
async fn setup_db() -> SqlitePool {
    ...
    sqlx::query(
        "CREATE TABLE stream_event (
            ...
            amount REAL NOT NULL, status TEXT NOT NULL DEFAULT 'projected',
    ...
    sqlx::query(
        "CREATE TABLE portfolio_snapshot (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            snapshot_date TEXT NOT NULL UNIQUE, portfolio_value REAL,
            portfolio_yield REAL, portfolio_count INTEGER, ytd_interest REAL,
            ytd_principal REAL, trust_balance REAL, outstanding_checks REAL,
            service_fees REAL,
    ...
    sqlx::query(
        "CREATE TABLE tmo_loan (
            loan_account TEXT PRIMARY KEY, stream_id INTEGER REFERENCES stream(id),
            ...
            appraised_value REAL, ltv REAL, percent_owned REAL, loan_type INTEGER,
            note_rate REAL, original_balance REAL, principal_balance REAL,
            regular_payment REAL, payment_frequency TEXT DEFAULT 'Monthly',
    ...
    sqlx::query("INSERT INTO stream (id, name, type) VALUES (1, 'Trustee Income', 'mortgage_portfolio')")
        .execute(&pool)
        .await
        .unwrap();

    // Create an expenses stream
    sqlx::query("INSERT INTO stream (id, name, type) VALUES (2, 'Expenses', 'manual')")
        .execute(&pool)
        .await
        .unwrap();
...
async fn test_forecast_running_sum() {
    ...
    insert_event(&pool, 1, "Lee payment", "2026-04-15", 396.50, "projected", "schedule").await;
    insert_event(&pool, 1, "Nakagawa payment", "2026-04-20", 1063.00, "projected", "schedule").await;
    insert_event(&pool, 2, "Car insurance", "2026-04-18", -1200.00, "projected", "manual").await;
    insert_event(&pool, 1, "Carvajal payment", "2026-05-01", 520.00, "projected", "schedule").await;
    insert_event(&pool, 2, "Rent", "2026-05-01", -2000.00, "projected", "manual").await;
...
async fn test_forecast_no_starting_balance() {
    ...
    let result = trust_deeds::db::forecasts::compute_forecast(
        &pool,
        "2026-04-01",
        "2026-06-01",
        None,
...
async fn test_cleanup_stale_projections() {
    ...
    insert_event(&pool, 1, "Stale event", "2020-01-01", 100.0, "projected", "schedule").await;
    // Create a future projected event (should survive)
    insert_event(&pool, 1, "Future event", "2099-01-01", 200.0, "projected", "schedule").await;
    // Create a past received event (should survive - it's not projected)
    insert_event(&pool, 1, "Past received", "2020-06-01", 300.0, "received", "tmo_history").await;
    ...
    for i in 0..3 {
        ...
        sqlx::query(
            "INSERT INTO stream_event (stream_id, label, scheduled_date, actual_date, amount, status, source_id, source_type, metadata)
             VALUES (1, 'Test', '2026-01-15', '2026-01-15', 400.0, 'received', ?, 'tmo_history', ?)",
        )
        .bind(&source_id)
...
async fn test_create_event() {
    ...
    let (amount,): (f64,) = sqlx::query_as("SELECT amount FROM stream_event WHERE id = ?")
        .bind(id)
        .fetch_one(&pool)
        .await
        .unwrap();
...
async fn test_override_event_date() {
    ...
    let id = insert_event(&pool, 1, "Lee payment", "2026-04-15", 396.50, "projected", "schedule");

    let updated = trust_deeds::db::events::override_event_date(
        &pool,
        id.await,
        "2026-04-22",
    )
    ...
    let (expected,): (Option<String>,) =
        sqlx::query_as("SELECT expected_date FROM stream_event WHERE id = ?")
            .bind(1i64)
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn test_cannot_override_received_event() {
    ...
    let id = insert_event(&pool, 1, "Lee payment", "2026-04-15", 396.50, "received", "tmo_history").await;
...
async fn test_coalesce_ordering() {
    ...
    insert_event(&pool, 1, "Event A", "2026-04-15", 100.0, "projected", "schedule").await;
    // Event B: scheduled Apr 10, but overridden to Apr 20
    let id_b = insert_event(&pool, 1, "Event B", "2026-04-10", 200.0, "projected", "schedule").await;
    sqlx::query("UPDATE stream_event SET expected_date = '2026-04-20' WHERE id = ?")
        .bind(id_b)
        .execute(&pool)
        .await
        .unwrap();
...
async fn test_forecast_stream_filter() {
    ...
    insert_event(&pool, 1, "Inflow", "2026-04-15", 500.0, "projected", "schedule").await;
    insert_event(&pool, 2, "Expense", "2026-04-20", -300.0, "projected", "manual").await;
...
async fn test_set_and_get_starting_balance() {
    ...
    // Initially none
    assert!(trust_deeds::db::forecasts::get_starting_balance(&pool).await.is_none());

    // Set it
    trust_deeds::db::forecasts::set_starting_balance(&pool, 12345.67).await.unwrap();
    ...
    // Update it
    trust_deeds::db::forecasts::set_starting_balance(&pool, 99999.99).await.unwrap();
    let balance = trust_deeds::db::forecasts::get_starting_balance(&pool).await;
