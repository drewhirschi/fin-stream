...
### 1. Cloudflare Tunnel for webhooks — **done**
- Tunnel `fin-stream-webhooks` created in Zero Trust; 4 edge connections healthy
- `cloudflared` Coolify service (`wehbooks-cloudflared`, uuid `t7djtak7o7rdwrmysu3slj9i`) running with `CF_TUNNEL_TOKEN` set (container renamed `cloudflared-<uuid>`)
- DNS: CNAME `finstream-webhooks.hirschi.dev → dc408b55-4b2c-4460-aa5f-a389da1865c8.cfargotunnel.com` (at external registrar; `hirschi.dev` added as pending Cloudflare zone without NS change)
- Published Application Route: `finstream-webhooks.hirschi.dev/webhooks/*` → `http://coolify-proxy:80` with `Host: finstream-webhooks.hirschi.dev` override. Non-matching paths fall through to the tunnel's default 404.
- Coolify app domain: registered as `http://finstream-webhooks.hirschi.dev` (note: `http://`, not `https://` — prevents Traefik's HTTP→HTTPS redirect loop since Cloudflare already terminates TLS at the edge)
- Verified: `POST /webhooks/resend` reaches the app; `GET /` and `/api/forecast` return 404 from CF without reaching the app; Tailscale `/health` still works.

### 2. Multi-arch Docker build — **partial**
- GitHub Actions CI build matrix works (amd64+arm64)
- Local `make build` not verified; `make ship` pipeline intact
- `docker login ghcr.io` uses `gh auth token | docker login ghcr.io -u drewhirschi --password-stdin` (existing `gh` CLI token already has `write:packages`)
- **Gotcha found:** commit `10d62e0` was not pushed to `origin/main` for 2 days, so CI never built it and `ghcr.io/.../fin-stream:latest` lagged by 2 commits. An amd64-only local build + push on 2026-04-16 corrected this. Push commits before relying on CI.

### 3. `iptables-persistent` cleanup — **not done**
- Still installed on `gory`, no custom rules active. Harmless.

### 4. `DATABASE_URL` verification — **done**
- Local `.env` `DATABASE_URL` resolves and connects.

### Still outstanding (moved out of this plan's original scope)
- Production env vars missing on the Coolify app: `RESEND_WEBHOOK_SECRET`, `RESEND_API_KEY`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `APP_ENCRYPTION_KEY`. See `session-auth.md` and `resend-email-receiving.md` for what each unblocks.
